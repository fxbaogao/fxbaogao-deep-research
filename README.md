...
本项目专门对接发现报告（fxbaogao）接口，支持研报搜索、详情拉取、PDF 下载地址获取、核心材料筛选、事实卡片沉淀和最终调研报告生成。
...
硬性规则：调用搜索、详情或下载接口前，必须先产出可审阅的研究边界和 MECE 子问题搜索矩阵。
...
| **接口直连** | 通过发现报告 API 搜索、查详情和获取 PDF 下载地址 |
...
默认情况下，轻量研究可以使用 detail 接口返回的摘要、目录和正文摘录。正式研究、需要引用原文或需要更高可复核性时，应通过下载接口获取 PDF 地址，并基于 PDF 原文核对。
...
**Q: detail 模式和 PDF 精读怎么选？**

A: 快速初筛或轻量研究可以用 detail 接口；正式研究、需要引用原文或需要更高可复核性时，用 PDF 原文。

**Q: 可以只研究一篇报告吗？**

A: 可以。直接调用详情接口，按 `REPORT_READER_精读分析.md` 模板精读即可。
